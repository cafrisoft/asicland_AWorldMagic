...
#ifndef PROJECTLIBRARYHANDLER_H
#define PROJECTLIBRARYHANDLER_H
...
//#include <KactusAPI/ProjectLoader.h>
...
#include <KactusAPI/DocumentValidator.h>
...
//! ProjectLibraryHandler is the class that implements the services to manage the IP-XACT library.
//-----------------------------------------------------------------------------
//class KACTUS2_API ProjectLibraryHandler : public QObject, public LibraryInterface
class ProjectLibraryHandler : public QObject, public LibraryInterface
{
    Q_OBJECT

public:

    static ProjectLibraryHandler* getInstance();

    static void initialize(MessageMediator* messageChannel, QObject* parent = nullptr);

    //! No copying
    ProjectLibraryHandler(const ProjectLibraryHandler &other) = delete;

    //! The destructor
    virtual ~ProjectLibraryHandler() = default;

    //! No assignment
    ProjectLibraryHandler &operator=(const ProjectLibraryHandler &other) = delete;
    ...
    */
    ...
    ProjectLibraryTreeModel* getTreeModel();
    ...
    virtual const QString getType() const override final;


    bool writeProjectToFile(Project const& project, VLNV const& vlnv, QString const& directory);
...
signals:
    ...
    //! Signal that user wants to open specified abstraction for editing
    void openAbsDef(VLNV const& busDefVLNV, VLNV const& absDefVLNV, bool disableBusDef);
 ...
 */
    ProjectLibraryHandler(MessageMediator* messageChannel, QObject* parent = nullptr);
    ...
    //! Loads all available VLNVs into the project cache.
    void loadAvailableVLNVs();

    //! Loads all available verilog into the project cache.
    void loadAvailableVerilogs();
...
//    static LibraryHandler* instance_;
    static ProjectLibraryHandler* ProjectLibraryHandler_instance_;
    ...
    ProjectLibraryTreeModel* treeModel_;
