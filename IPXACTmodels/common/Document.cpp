...
//-----------------------------------------------------------------------------
// Function: Document::setAuthor()
//-----------------------------------------------------------------------------
void Document::setAuthor(QString const& author)
{
    foreach(QSharedPointer<VendorExtension> extension, *getVendorExtensions())
    {
        if (extension->type() == QLatin1String("kactus2:author"))
        {
            extension.dynamicCast<Kactus2Value>()->setValue(author);
            return;
        }
    }

    getVendorExtensions()->append(QSharedPointer<Kactus2Value>(new Kactus2Value(QStringLiteral("kactus2:author"),
        author)));
