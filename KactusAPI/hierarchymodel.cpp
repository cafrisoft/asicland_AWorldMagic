...
QAbstractItemModel(parent),
    ...
    handler_(handler)
{
    connect(rootItem_, SIGNAL(inputMessage(const QString&)),
...
QVariant HierarchyModel::data(QModelIndex const& index, int role) const
{
    ...
    else if (role == Qt::DecorationRole && index.column() == HierarchyModel::OBJECT_COLUMN)
    {
        if (item->type() == HierarchyItem::COMPONENT)
        {
            ...
            {
                return QIcon(QStringLiteral(":/resources/Res/system-component.png"));
            }
            else if (item->getImplementation() == KactusAttribute::SW)
            {
                if (item->isHierarchical())
                {
                    return QIcon(QStringLiteral(":/resources/Res/hier-sw-component.png"));
                }
                else
                {
                    return QIcon(QStringLiteral(":/resources/Res/sw-component48x48.png"));
                }
            }
            else
            {
                if (item->isHierarchical()) 
                {
                    return QIcon(QStringLiteral(":/resources/Res/hier-hw-component.png"));
                }
                else
                {
                    return QIcon(QStringLiteral(":/resources/Res/hw-component.png"));
                }		
            }
        }
        else if (item->type() == HierarchyItem::CATALOG)
        {
            return QIcon(QStringLiteral(":/resources/Res/catalog.png"));
        }
        else if (item->type() == HierarchyItem::COMDEFINITION)
        {
            return QIcon(QStringLiteral(":/resources/Res/new-com_definition.png"));
        }
        else if (item->type() == HierarchyItem::APIDEFINITION)
        {
            return QIcon(QStringLiteral(":/resources/Res/new-api_definition.png"));
        }
    	else if (item->type() == HierarchyItem::HW_DESIGN)
        {
            return QIcon(QStringLiteral(":/resources/Res/hw-design.png"));
    	}
    	else if (item->type() == HierarchyItem::SW_DESIGN || item->type() == HierarchyItem::SYS_DESIGN)
        {
            return QIcon(QStringLiteral(":/resources/Res/sw-design48x48.png"));
    	}
        else if (item->type() == HierarchyItem::ABSDEFINITION)
        {
            return QIcon(QStringLiteral(":/resources/Res/abs-def.png"));
        }
        else if (item->type() == HierarchyItem::BUSDEFINITION)
        {
            return QIcon(QStringLiteral(":/resources/Res/bus-def.png"));
        }
        else
        {
            return QIcon(QStringLiteral(":/resources/Res/cross.png"));
        }
    }
