...
//-----------------------------------------------------------------------------
GenerationControl::GenerationControl(LibraryInterface* designInterface,
    ...
    input_(input),
    settings_(settings),
    isDesignGeneration_(input.design != 0),
...
bool GenerationControl::writeDocuments()
{
    ...
    QString componentPath = designInterface_->getPath(input_.component->getVlnv());
...
bool GenerationControl::saveChanges()
{
    ...
    // Try to save.
    bool saveSucceeded = designInterface_->writeModelToFile(input_.component);
    ...
    QString savePath = designInterface_->getPath(input_.component->getVlnv());
...
void GenerationControl::initializeDocuments()
{
    ...
    if (isDesignGeneration_)
    {
        ...
        QList<QSharedPointer<MetaDesign> > designs =
            MetaDesign::parseHierarchy(designInterface_, libraryInterface_, input_, viewSelection_->getView());
...
QString GenerationControl::defaultOutputPath() const
{
    ...
    QString topComponentPath = designInterface_->getPath(input_.component->getVlnv());
...
{
    QString xmlPath = designInterface_->getPath(input_.component->getVlnv());
