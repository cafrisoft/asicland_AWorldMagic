	...
	for (auto const& fileSet : *component->getFileSets())
	{
		for (auto const& file : *fileSet->getFiles())
		{
			if (isOfSupportedFileType(file))
			{
                ...
                QMap<QString, QString> itemsInFile = findItemsInFileContent(content, path);

                for (auto it = itemsInFile.begin(); it != itemsInFile.end(); ++it) {
                    itemsInFilesets.insert(it.key(), it.value());
                }
