...
//-----------------------------------------------------------------------------
MetaDesign::MetaDesign(LibraryInterface* designInterface,
...
//-----------------------------------------------------------------------------
QList<QSharedPointer<MetaDesign> > MetaDesign::parseHierarchy(LibraryInterface* designInterface,
                                                              LibraryInterface* libraryInterface,
                                                              ...
                                                              QSharedPointer<View> topComponentView)
{
    ...
    QSharedPointer<MetaDesign> topMostDesign(new MetaDesign(designInterface,
                                                            libraryInterface,
                                                            input.messages,
                                                            input.design,
                                                            designInstantiation,
...
//        QSharedPointer<Component> component = libraryInterface_->getModel<Component>(instanceVLNV);
        QSharedPointer<Component> component;
        component = designInterface_->getModel<Component>(instanceVLNV);

        if (!component)
        {
            component = libraryInterface_->getModel<Component>(instanceVLNV);
...
void MetaDesign::findHierarchy(QSharedPointer<MetaInstance> mInstance)
{
    ...
    if (subDesign)
    {
        ...
        QSharedPointer<MetaDesign> subMetaDesign(new MetaDesign(designInterface_,
                                                                libraryInterface_,
                                                                messages_,
                                                                subDesign,
                                                                designInstantiation,
                                                                subDesignConfiguration,
                                                                mInstance));
    ...
    QSharedPointer<DesignConfigurationInstantiation> configurationInstantiation)
{
    ...
    if (configurationInstantiation && configurationInstantiation->getDesignConfigurationReference())
    {
        ...
        referredDesignConfiguration = designInterface_->getModel<DesignConfiguration>(
    ...
    QSharedPointer<DesignConfiguration> configuration)
{
    ...
    {
        referredDesign = designInterface_->getModel<Design>(*(designInstantiation->getDesignReference()));
        if (!referredDesign)
        {
    ...
    if (configuration)
    {
        ...
        {
            referredDesign = designInterface_->getModel<Design>(configuration->getDesignRef());
        }
    }
