...
#include <UI/Common/ImageExporter.h>
#include <UI/Common/DesignDiagram.h>
#include <UI/Common/StickyNote/StickyNote.h>
#include <UI/Common/StickyNote/StickyNoteRemoveCommand.h>
...
// NOTE : Design screen which include schematic's general functions.

//-----------------------------------------------------------------------------
// Function: DesignWidget::DesignWidget()
//-----------------------------------------------------------------------------
DesignWidget::DesignWidget(LibraryInterface* designInterface, LibraryInterface* libraryInterface, QWidget* parent):
    ...
    diagram_(0)
{
    ...
    whatWindowIsThis_ = TabDocument::SCHEMATIC;
...
void DesignWidget::setDiagram(DesignDiagram* diagram)
{
    ...
    connect(diagram_, SIGNAL(inputMessage(const QString&)),
        this, SIGNAL(inputMessage(const QString&)), Qt::UniqueConnection);
...
void DesignWidget::refresh()
{
    ...
    LibraryInterface* libraryInterface = designInterface_;
    VLNV vlnv = editedComponent_->getVlnv();
    ...
    QSharedPointer<Component> comp = libraryInterface->getModel(vlnv).staticCast<Component>();
...
bool DesignWidget::save()
{
    ...
    designInterface_->beginSave();

    bool writeSucceeded = true;

    // Write the files.
    if (designConf && !designInterface_->writeModelToFile(designConf))
    {
        writeSucceeded = false;
    }

    if (!designInterface_->writeModelToFile(design))
    {
        writeSucceeded = false;
    }

    if (!designInterface_->writeModelToFile(editedComponent_))
    {
        writeSucceeded = false;
    }

    designInterface_->endSave();
...
void DesignWidget::test(QSharedPointer<Component> component, const QString& viewName)
{

//    QSharedPointer<Component> component = designInterface_->getModel<Component>(componentVLNV);


//    for (QSharedPointer<BusInterface> const& busInterface : *component->getBusInterfaces())
...
//-----------------------------------------------------------------------------
// Function: DesignWidget::getInterface()
//-----------------------------------------------------------------------------
LibraryInterface* DesignWidget::getInterface(const VLNV& vlnv) const
{
...
//-----------------------------------------------------------------------------
// Function: DesignWidget::getDesignInterface()
//-----------------------------------------------------------------------------
LibraryInterface* DesignWidget::getDesignInterface() const
{
...
//-----------------------------------------------------------------------------
LibraryInterface* DesignWidget::getLibraryInterface() const
{
...
bool DesignWidget::exportImage()
{
    ...
    QString libraryPath = designInterface_->getDirectoryPath(designVLNV);
