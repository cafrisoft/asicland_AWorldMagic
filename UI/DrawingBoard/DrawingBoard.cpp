...
{    
...
#if PROTECTION_ENABLE_20231229 == 0
    doc->setProtection(doc->getDocumentVLNV().getVersion() != "draft");
#else
    doc->setProtection(false);
#endif

    connect(doc, SIGNAL(inputMessage(QString const&)), this, SIGNAL(inputMessage(QString const&)), Qt::UniqueConnection);
    connect(doc, SIGNAL(noticeMessage(QString const&)), this, SIGNAL(noticeMessage(QString const&)), Qt::UniqueConnection);
    connect(doc, SIGNAL(errorMessage(QString const&)), this, SIGNAL(errorMessage(QString const&)), Qt::UniqueConnection);
    connect(doc, SIGNAL(undefinedMessage(QString const&)), this, SIGNAL(undefinedMessage(QString const&)), Qt::UniqueConnection);

    connect(doc, SIGNAL(helpUrlRequested(QString const&)), this, SIGNAL(helpUrlRequested(QString const&)), Qt::UniqueConnection);
    ...
    connect(doc, SIGNAL(documentSaved(TabDocument*)), this, SLOT(onDocumentSaved(TabDocument*)), Qt::UniqueConnection);
    connect(doc, SIGNAL(titleChanged(TabDocument*, QString const&)), this, SLOT(onDocumentTitleChanged(TabDocument*, QString const&)), Qt::UniqueConnection);
    ...
    if (doc->getWhatWindowIsThis() == TabDocument::SCHEMATIC)
    {
        QString doc_vendor = doc->getDocumentVLNV().getVendor();
        QString doc_library = doc->getDocumentVLNV().getLibrary();
        QString doc_name = doc->getDocumentVLNV().getName();
        QString doc_version = doc->getDocumentVLNV().getVersion();

        emit inputMessage("open design " + doc_vendor + " " + doc_library + " " + doc_name + " " + doc_version);

        emit noticeMessage("[ SYSTEM ] | Open | Design (Schematic) opened.");
    }

    else if (doc->getWhatWindowIsThis() == TabDocument::TEXTEDITOR)
    {
        QString doc_file = doc->getDocumentName();

        emit inputMessage("open file " + doc_file);

        emit noticeMessage("[ SYSTEM ] | Open | File opened.");
    }
...
//-----------------------------------------------------------------------------
// Function: DrawingBoard::addAndOpenDocument_Console()
//-----------------------------------------------------------------------------
void DrawingBoard::addAndOpenDocument_Console(TabDocument* doc)
{
    // Open in unlocked mode by default only if the version is draft.
#if PROTECTION_ENABLE_20231229 == 0
    doc->setProtection(doc->getDocumentVLNV().getVersion() != "draft");
    ...
    if (doc->getSupportedWindows() == TabDocument::VENDOREXTENSIONWINDOW)
    {
        emit noticeMessage("[ SYSTEM ] | Open | Vendor Extension opened.");
    }
...
void DrawingBoard::saveCurrentDocument()
{
    ...
    if (doc != 0 && shouldSave(doc))
    {
        ...
        QString resultText = "[ SYSTEM ] | Save | File Saved";
...
void DrawingBoard::saveAll()
{
    ...
    QString resultText = "[ SYSTEM ] | Save All | All files Saved";
...
{	
    ...
    if (document->getWhatWindowIsThis() == TabDocument::SCHEMATIC)
    {
        QString doc_vendor = document->getDocumentVLNV().getVendor();
        QString doc_library = document->getDocumentVLNV().getLibrary();
        QString doc_name = document->getDocumentVLNV().getName();
        QString doc_version = document->getDocumentVLNV().getVersion();

        emit inputMessage("close design " + doc_vendor + " " + doc_library + " " + doc_name + " " + doc_version);

        emit noticeMessage("[ SYSTEM ] | Close | Design (Schematic) closed.");
    }

    else if (document->getWhatWindowIsThis() == TabDocument::TEXTEDITOR)
    {
        QString doc_file = document->getDocumentName();

        emit inputMessage("close file " + doc_file);

        emit noticeMessage("[ SYSTEM ] | Close | File closed.");
    }
...
//-----------------------------------------------------------------------------
// Function: DrawingBoard::closeAndRemoveDocument_Console()
//-----------------------------------------------------------------------------
void DrawingBoard::closeAndRemoveDocument_Console(int index)
{
    Q_ASSERT(widget(index) != 0);
    TabDocument* document = static_cast<TabDocument*>(widget(index));
