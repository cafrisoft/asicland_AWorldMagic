...
#include <UI/HWDesign/undoCommands/ColumnDeleteCommand.h>
#include <UI/HWDesign/undoCommands/ComponentDeleteCommand.h>
#include <UI/HWDesign/undoCommands/ConnectionDeleteCommand.h>
#include <UI/HWDesign/undoCommands/InterfaceDeleteCommand.h>
#include <UI/HWDesign/undoCommands/PortDeleteCommand.h>
...
//-----------------------------------------------------------------------------
HWDesignWidget::HWDesignWidget(LibraryInterface *designInterface,
    ...
    DesignWidget(designInterface, libraryInterface, parent),
    ...
    expressionParser_()
{
    ...
    setDiagram(new HWDesignDiagram(designInterface,
                                   libraryInterface,
                                   getEditProvider(),
                                   designAndIsntancesParameterFinder,
                                   designParameterFinder, this));
...
{	
	...
	if (vlnv.isValid() && !viewName.isEmpty())
    {
        ...
        if (vlnv.isValid() && vlnv.getType() == VLNV::COMPONENT) {
			...
			// create model 
            QSharedPointer<Document> libComponent = getDesignInterface()->getModel(vlnv);
    ...
    {
        Q_ASSERT(getDesignInterface()->contains(vlnv));
        Q_ASSERT(getDesignInterface()->getDocumentType(vlnv) == VLNV::COMPONENT);
        ...
        const QString xmlPath = getDesignInterface()->getPath(vlnv);
...
//    setDocumentName(QString("[%3] %1 (%2)").arg(getIdentifyingVLNV().getName()).arg(getIdentifyingVLNV().getVersion()).arg(designInterface_->getType()));
...
bool HWDesignWidget::setDesign(QSharedPointer<Component> component, QString const& viewName)
{
    ...
    if (!openView->getDesignConfigurationInstantiationRef().isEmpty())
    {
        ...
        designConfiguration = getDesignInterface()->getModel(configurationVLNV).dynamicCast<DesignConfiguration>();
        if (designConfiguration)
        {
            design = getDesignInterface()->getModel(designConfiguration->getDesignRef()).dynamicCast<Design>();
        }
...
//            emit errorMessage(tr("Component %1 did not contain a hierarchical view").arg(component->getVlnv().getName()));
    ...
    else if (!openView->getDesignInstantiationRef().isEmpty())
    {
        ...
        design = getDesignInterface()->getModel(designVLNV).dynamicCast<Design>();
        if (!design)
        {
...
bool HWDesignWidget::saveAs()
{
    ...
    if (!NewObjectDialog::saveAsDialog(
        this, getDesignInterface(), oldVLNV, prodHier, firmness, tags, vlnv, directory))
    {
        return false;
    }
    ...
    QFileInfo sourceInfo(getDesignInterface()->getPath(oldComponent->getVlnv()));
    ...
    getDesignInterface()->beginSave();

	// if design configuration is used then write it.
    if (designConf && !getDesignInterface()->writeModelToFile(directory, designConf))
    {
	    writeSucceeded = false;
	}

    if (!getDesignInterface()->writeModelToFile(directory, design))
    {
		writeSucceeded = false;
	}

    if (!getDesignInterface()->writeModelToFile(directory, topComponent))
    {
		writeSucceeded = false;
	}

    getDesignInterface()->endSave();

	if (writeSucceeded)
    {
//        setDocumentName("[" + designInterface_->getType() + "] }" +getEditedComponent()->getVlnv().getName() + " (" +
...
{
    // Handler delete events if the document is not protected.
...
{    
    ...
    if (prevlnv.isValid() && getDesignInterface()->contains(prevlnv))
    {
		vlnv = prevlnv;
        path = getDesignInterface()->getPath(prevlnv);
    ...
    {
        NewObjectDialog newDesignDialog(getDesignInterface(), VLNV::COMPONENT, true, parentWidget());
    ...
    if (getDesignInterface()->contains(prevlnv))
    {
		// find the component
        QSharedPointer<Document> libComp = getDesignInterface()->getModel(prevlnv);
    ...
    getDesignInterface()->writeModelToFile(path, newComponent);
...
void HWDesignWidget::createDesignForComponent(QSharedPointer<Component> component, const QString& dirPath)
{
    ...
    while (getDesignInterface()->contains(designVLNV))
    {
		++runningNumber;
	...
	// if vlnv is reserved then add "(<number>)" to end of version field
    while (getDesignInterface()->contains(desConfVLNV))
    {
		++runningNumber;
    ...
    getDesignInterface()->writeModelToFile(dirPath, newDesign);
    getDesignInterface()->writeModelToFile(dirPath, designConf);
    getDesignInterface()->writeModelToFile(component);
