...
void LibraryTreeWidget::setupConnections(LibraryTreeModel* dataModel)
{
    ...
    connect(&view_, SIGNAL(createNewBus()),
        this, SIGNAL(createNewBus()), Qt::UniqueConnection);

    connect(&view_, SIGNAL(createNewHWComponent()),
        this, SIGNAL(createNewHWComponent()), Qt::UniqueConnection);
    ...
    connect(&view_, SIGNAL(changeDesignProtection(bool const&)),
        this, SIGNAL(changeDesignProtection(bool const&)), Qt::UniqueConnection);

	connect(dataModel, SIGNAL(invalidateFilter()), filter_, SLOT(invalidate()), Qt::UniqueConnection);
}

void LibraryTreeWidget::onChangeDesignProtection(bool const &locked)
{
    emit changeDesignProtection(locked);
}
