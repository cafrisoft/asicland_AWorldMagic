...
//! The widget to display the library in a tree.
//-----------------------------------------------------------------------------
class ProjectTreeView : public ExpandingTreeView
{
	Q_OBJECT
...
public:
	...
	*
	*/
        ProjectTreeView(LibraryInterface* handler, ProjectTreeFilter* filter, QWidget* parent = 0);
...
private:
    ...
    //! No assignment
        ProjectTreeView(const ProjectTreeView& other);

	//! No copying
        ProjectTreeView& operator=(const ProjectTreeView& other);
