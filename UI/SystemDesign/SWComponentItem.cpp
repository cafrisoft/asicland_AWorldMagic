...
#include <UI/Common/graphicsItems/IGraphicsItemStack.h>
...
//-----------------------------------------------------------------------------
SWComponentItem::SWComponentItem(LibraryInterface* designInterface,
                                 LibraryInterface* libInterface,
                                 QSharedPointer<Component> component,
                                 QSharedPointer<ComponentInstance> instance):
    SystemComponentItem(QRectF(-COMPONENTWIDTH / 2, 0, COMPONENTWIDTH, MIN_HEIGHT), designInterface, libInterface, instance, component, 0),
    oldStack_(0),
    oldPos_(),
    hierIcon_(0),
    importedIcon_(0),
...
void SWComponentItem::updateComponent()
{
    ...
    if (componentModel()->isHierarchical())
    {
        if (hierIcon_ == 0)
        {
            hierIcon_ = new QGraphicsPixmapItem(QPixmap(":resources/Res/hierarchy.png"), this);
    ...
    if (isImported())
    {
        if (importedIcon_ == 0)
        {
            hierIcon_ = new QGraphicsPixmapItem(QPixmap(":resources/Res/imported.png"), this);
