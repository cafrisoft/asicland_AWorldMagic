...
#include <UI/Common/graphicsItems/GraphicsColumnAddCommand.h>
...
#include <UI/SystemDesign/SystemDetailsEditor/SwitchHWDialog.h>
#include <UI/SystemDesign/ComGraphicsConnection.h>
#include <UI/SystemDesign/ApiGraphicsConnection.h>
#include <UI/SystemDesign/UndoCommands/ApiConnectionAddCommand.h>
#include <UI/SystemDesign/UndoCommands/ComConnectionAddCommand.h>
#include <UI/SystemDesign/UndoCommands/SWInterfaceAddCommand.h>
#include <UI/SystemDesign/UndoCommands/SystemChangeCommands.h>
#include <UI/SystemDesign/UndoCommands/SystemMoveCommands.h>
#include <UI/SystemDesign/UndoCommands/SystemAddCommands.h>
#include <UI/SystemDesign/UndoCommands/SystemComponentAddCommand.h>
#include <UI/SystemDesign/UndoCommands/ReplaceSystemComponentCommand.h>
...
//-----------------------------------------------------------------------------
SystemDesignDiagram::SystemDesignDiagram(bool onlySW,
...
void SystemDesignDiagram::onAddToLibraryAction()
{
    if (singleSelection())
    {
        ...
        if (item != 0 && item->type() == SWComponentItem::Type)
        {
            ...
            NewObjectDialog dialog(getDesignInterface(), VLNV::COMPONENT, false, getParent());
            ...
            getDesignInterface()->writeModelToFile(dialog.getPath(), comp->componentModel());
...
void SystemDesignDiagram::mouseDoubleClickEvent(QGraphicsSceneMouseEvent* event)
{
    ...
    if (dynamic_cast<SystemComponentItem*>(item) != 0)
    {
        ...
        if (getDesignInterface()->contains(comp->componentModel()->getVlnv()))
        {
...
void SystemDesignDiagram::dragEnterEvent(QGraphicsSceneDragDropEvent * event)
{
    ...
    if (!isProtected() && event->mimeData()->hasImage())
    {
        ...
        if (vlnv.getType() == VLNV::COMPONENT)
        {
            ...
            QSharedPointer<Document> libComp = getDesignInterface()->getModel(vlnv);
...
void SystemDesignDiagram::dropEvent(QGraphicsSceneDragDropEvent *event)
{
    ...
    if (dragType_ == DRAG_TYPE_SW)
    {
        ...
        QSharedPointer<Document> libComp = getDesignInterface()->getModel(droppedVLNV);
        QSharedPointer<Component> comp = libComp.staticCast<Component>();
        ...
        if (event->dropAction() == Qt::CopyAction)
        {
            ...
            if (stack != 0)
			{
                ...
                SWComponentItem* item = new SWComponentItem(getDesignInterface(), getLibraryInterface(), comp, swInstance);
        ...
        else if (event->dropAction() == Qt::MoveAction)
        {
			...
			// Create the SW component item.
            SWComponentItem* newCompItem = new SWComponentItem(getDesignInterface(), getLibraryInterface(), comp, swInstance);
 ...
 QMessageBox::Ok, getParent());
        ...
        SwitchHWDialog dialog(newComponent, viewName, getDesignInterface(), getParent());
        ...
        if (dialog.isCopyActionSelected())
        {
            ...
            getDesignInterface()->writeModelToFile(dialog.getPath(), design);
            getDesignInterface()->writeModelToFile(dialog.getPath(), desConf);
        ...
        else
        {
            ...
            getDesignInterface()->writeModelToFile(getEditedComponent());
        }

        // Add the system view to the new HW component and save.
        newComponent->getVendorExtensions()->append(newView);
        getDesignInterface()->writeModelToFile(newComponent);
    ...
    else if (dragType_ == DRAG_TYPE_DEFINITION)
    {
        if (dragEndPoint_ != 0)
        {
            Q_ASSERT(getDesignInterface()->contains(droppedVLNV));

            VLNV vlnv = droppedVLNV;
            vlnv.setType(getDesignInterface()->getDocumentType(droppedVLNV));
...
void SystemDesignDiagram::loadDesign(QSharedPointer<Design> design)
{
    ...
    if (!onlySW_)
    {
        ...
        {
            QSharedPointer<Document> libComponent = getDesignInterface()->getModel(*instance->getComponentRef());
...
//                    "the library").arg(instance->getComponentRef()->getName(), design->getVlnv().getName()));
            ...
            HWMappingItem* item = new HWMappingItem(getDesignInterface(), getLibraryInterface(), component, instance); //instance->getConfigurableElementValues());
    ...
    foreach (QSharedPointer<ComponentInstance> instance, *design->getComponentInstances())
    {
        ...
        {
             component = getDesignInterface()->getModel(*instance->getComponentRef()).dynamicCast<Component>();
...
//                     "the library").arg(instance->getComponentRef()->getName(), design->getVlnv().getName()));
        ...
        SWComponentItem* item = new SWComponentItem(getDesignInterface(), getLibraryInterface(), component, instance);
        connect(item, SIGNAL(openCSource(ComponentItem*)), this, SIGNAL(openCSource(ComponentItem*)));
        connect(item, SIGNAL(errorMessage(QString const&)), this, SIGNAL(errorMessage(QString const&)));
    ...
    else if (item == 0 || item->type() == HWMappingItem::Type)
    {
        ...
        if (stack != 0)
        {
            if (stack->getContentType() == ColumnTypes::COMPONENTS)
            {
                ...
                SWComponentItem* swCompItem = new SWComponentItem(getDesignInterface(), getLibraryInterface(), comp, swInstance);
                swCompItem->setDraft();
                                           ...
                                           QUndoCommand* cmd, bool useCursorPos)
{
    foreach (ComponentInstanceCopyData const instanceCopy, collection.instances)
    {
        ...
        SWComponentItem* comp = new SWComponentItem(getDesignInterface(), getLibraryInterface(), component, swInstanceCopy);
