...
TabDocument::TabDocument(QWidget* parent, unsigned int flags, int minZoomLevel, int maxZoomLevel) : 
    QWidget(parent),
    supportedWindows_(OUTPUTWINDOW | CONSOLEWINDOW | REPORTWINDOW | LIBRARYWINDOW | CONTEXT_HELP_WINDOW | TEXTEDITORWINDOW | WELCOMEWINDOW ),
    flags_(flags),
    modified_(false),
    locked_(false),
    zoomLevel_(100),
...
// Function: TabDocument::getSupportedWindows()
//-----------------------------------------------------------------------------
unsigned int TabDocument::getWhatWindowIsThis() const
{
    return whatWindowIsThis_;
}
