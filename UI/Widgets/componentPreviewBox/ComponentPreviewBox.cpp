...
void ComponentPreviewBox::updatePreview()
{
    ...
    if (component_ != 0)
    {
        ...
        if (component_->getImplementation() == KactusAttribute::HW)
        {
            ...
            item = new HWComponentItem(lh_, lh_, componentInstance, component_);
        }
        else if (component_->getImplementation() == KactusAttribute::SW)
		{
            ...
            item = new SWComponentItem(lh_, lh_, component_, swInstance);
        }
...
//-----------------------------------------------------------------------------
// Function: ComponentPreviewBox::updatePreview(LibraryInterface* lh)
//-----------------------------------------------------------------------------
void ComponentPreviewBox::updatePreview(LibraryInterface* lh)
{
    // Re-create the scene.
    delete scene_;
    scene_ = new GridScene(view_);
    ...
    if (component_ != 0)
    {
        ...
        if (component_->getImplementation() == KactusAttribute::HW)
        {
            ...
            item = new HWComponentItem(lh, lh, componentInstance, component_);
        }
        else if (component_->getImplementation() == KactusAttribute::SW)
        {
            QSharedPointer<ComponentInstance> swInstance(new ComponentInstance());
            ...
            item = new SWComponentItem(lh, lh, component_, swInstance);
        }
...
// Function: ComponentPreviewBox::setComponent()
//-----------------------------------------------------------------------------
void ComponentPreviewBox::setComponent(LibraryInterface* lh, QSharedPointer<Component> component)
{
    component_ = component;
    updatePreview(lh);
}
...
// Function: ComponentPreviewBox::setComponent()
//-----------------------------------------------------------------------------
void ComponentPreviewBox::setComponent( LibraryInterface* lh, const VLNV& vlnv )
{
    QSharedPointer<Component> component;

    // if the vlnv belongs to a component
    if (lh->getDocumentType(vlnv) == VLNV::COMPONENT)
    {
        component = lh->getModel(vlnv).staticCast<Component>();
    }
    setComponent(lh, component);
}
