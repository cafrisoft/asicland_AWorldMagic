...
#include <UI/Widgets/FileSelector/fileselector.h>

#include <UI/ComponentEditor/ports/portseditor.h>
...
componentXmlPath_(libraryInterface->getPath(component->getVlnv())),
component_(component),
importComponent_(new Component(*component_)),
selectedSourceFile_(),
  selectedSourceAbsolutePath_(),
  portEditor_(0),
fileSelector_(new FileSelector(component, this)),
editButton_(new QPushButton(QIcon(":/resources/Res/edit.png"), tr("Edit file"), this)),
refreshButton_(new QPushButton(QIcon(":/resources/Res/refresh.png"), tr("Refresh"), this)),
browseButton_(new QPushButton(this)),
componentSelector_(new QComboBox(this)),
sourceDisplayTabs_(new QTabWidget(this)),
runner_(new ImportRunner(designInterface, libraryInterface, parameterFinder, sourceDisplayTabs_, this)),
messageBox_(new QLabel(this)),
componentViews_(component->getViews())
{
    componentSelector_->setDisabled(true);

    browseButton_->setIcon(QIcon(":/resources/Res/opened-folder.png"));
    ...
    portEditor_ = new PortsEditor(importComponent_, libraryInterface, parameterFinder, expressionFormatter, portValidator,
    ...
    connect(fileSelector_, SIGNAL(fileSelected(const QString&, QSharedPointer<FileSet>)), this,
        SLOT(onFileSelected(const QString&, const QString&, QSharedPointer<FileSet>)), Qt::UniqueConnection);
...
//-----------------------------------------------------------------------------
void ImportEditor::onFileSelected(QString const& filePath, QString const& absolutePath, QSharedPointer<FileSet> fileSet)
{
    selectedSourceFile_ = filePath;
	selectedFileSet_ = fileSet;

    selectedSourceAbsolutePath_ = absolutePath;
...
void ImportEditor::onBrowseFile()
{
    ...
    if (browseDialog.exec() == QDialog::Accepted)
    {
        ...
        if (!selectedFiles.isEmpty())
        {
            ...
            QString absolutePath = selectedFiles.first();
            ...
            onFileSelected(relativePath, absolutePath, QSharedPointer<FileSet>());
        }
    }
}
...
//    QStringList componentNames =
//        runner_->constructComponentDataFromFile(selectedSourceFile_, selectedSourceAbsolutePath_, component_);
